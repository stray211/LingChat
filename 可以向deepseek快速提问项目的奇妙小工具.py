...
EXCLUDED_ITEMS = {'.git', '__pycache__', 'venv', 'env', '.venv', '.vscode', '.idea', '.DS_Store'}
...
def get_code_contents(root_dir, script_name):
    """
    获取项目中所有 .py 文件和 README.md 文件的内容。
    ...
    # 2. 递归查找所有 .py 文件和其他 README.md (如果在子目录)
    ...
    for relative_path_display, absolute_path in files_to_extract_info:
        try:
            ...
            lang_type = ""
            if relative_path_display.endswith(".py"):
                lang_type = "python"
            elif relative_path_display.lower().endswith(".md"):
                lang_type = "markdown" # 或者留空，让 Markdown 渲染器自行处理
...
def main():
    ...
    # 2. 获取代码内容
    print("正在提取 .py 和 README.md 文件内容...")
    try:
    ...
    try:
        with open(output_md_path, 'w', encoding='utf-8') as f:
            ...
            if code_contents_str:
                ...
                f.write("未找到 .py 或 README.md 文件，或提取失败。\n")
