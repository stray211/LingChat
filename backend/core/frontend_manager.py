...
class FrontendManager:
    ...
    def start_frontend(self, frontend_dir: str, port: str = "3000") -> bool:
        ...
        if not os.path.isfile(server_js_path):
            logger.error(f"前端脚本未找到: {server_js_path}")
            return False
        ...
        try:
            ...
            logger.info(f"前端服务器已启动 (PID: {self.process.pid})")
            
            time.sleep(3)  # 等待服务器启动
            if self.process.poll() is not None:
                logger.error(f"前端服务器退出，错误码: {self.process.poll()}")
                return False
            ...
            logger.error("未找到'node'命令. 请安装Node.js")
            return False
    ...
    def _register_cleanup(self):
        ...
        for sig in signals:
            try:
                ...
                logger.warning(f"无法注册信号 {sig}: {e}")

    def _handle_signal(self, sig, frame):
        """处理操作系统信号"""
        logger.info(f"收到信号 {signal.Signals(sig).name}，正在关闭...")
        self.stop_frontend()
        sys.exit(0)