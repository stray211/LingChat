...
function praseSaveError(command) {
  switch (command) {
    ...
    case "process-log": return "存档处理失败"
  }
}

function praseSaveSuccess(command) {
  switch (command) {
    case "list": return "存档列表读取成功"
    case "load": return "存档加载成功"
    case "save": return "存档保存成功"
    case "delete": return "存档删除成功"
    case "create": return "存档创建成功"
    case "process-log": return "存档处理成功"
  }
}

function praseSaveLoading(command) {
  switch (command) {
    case "list": return "正在读取存档列表"
    case "load": return "正在加载存档"
    case "save": return "正在保存存档"
    case "delete": return "正在删除存档"
    case "create": return "正在创建存档"
    case "process-log": return "正在处理存档"
  }
}
...
export const saveRequest = {
  ...
  list: (userId, page, pageSize) => {
    return saveRequest.sendGET('list', `
      ?user_id=${userId}
      &page=${page}
      &page_size=${pageSize}
    `)
  ...
  load: (userId, convoId) => {
    return saveRequest.sendGET('load', `
      ?user_id=${userId}
