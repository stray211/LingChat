...
class VitsTTS:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.api_url, timeout=2) as response:
                    if response.status < 500:
                        self._log_tts_status(True,
                                             f"vits-simple-api 服务可达 ({service_host_port}, 状态: {response.status})")
                        self.enable = True
                        return True
                    else:
    ...
    async def generate_voice(self, text, file_name, speaker_id, save_file=False):
        ...
        try:
            ...
            logger.error(
                f"语音生成HTTP请求失败 (URL: {e.request_info.url}, 状态: {e.status}, 消息: {e.message}) 文本: \"{text}\"")
