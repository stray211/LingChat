...
class RAGSystem:
    ...
    def initialize(self) -> bool:
        ...
        if not _sentence_transformer_imported_ok:
            logger.error(
                f"RAG组件初始化失败: 'sentence-transformers' 模块未找到。请运行: pip install sentence-transformers")
            return False
        ...
        try:
            ...
            logger.debug(
                f"RAG: 本地Sentence Transformer模型 ({self.EMBEDDING_MODEL_NAME}) 加载成功。当前使用 {device} 进行RAG向量库匹配的推理。")
    ...
    def add_session_to_history(self, session_messages: List[Dict], session_filepath: Optional[str] = None) -> Optional[
        str]:
        '''
        ...
        if len(filtered_messages) < len(session_messages) and logger.should_print_context():
            logger.debug(
                f"RAG: 根据session_id过滤后，消息数量从 {len(session_messages)} 减少到 {len(filtered_messages)}")
    ...
    def get_relevant_messages(self, query_text: str) -> List[Dict]:
        ...
        logger.debug(
            f"RAG: 检索参数: 目标块数={retrieval_count}, 候选倍数={candidate_multiplier}, 上文窗口={context_before}, 下文窗口={context_after}")
            ...
            if logger.should_print_context():
                ...
                for i in range(min(5, len(results['ids'][0]))):
                    try:
                        ...
                        content = results['documents'][0][i][:100] + "..." if len(results['documents'][0][i]) > 100 else \
                        results['documents'][0][i]
                        metadata = results['metadatas'][0][i]
                        logger.debug(
                            f"候选[{i + 1}]: 距离={distance:.4f}, 角色={metadata.get('role', 'unknown')}, ID={doc_id}")
            ...
            for i in range(len(results['ids'][0])):
                ...
                if potential_block_messages:
                    ...
                    if logger.should_print_context():
                        ...
                        logger.debug(
                            f"RAG: 上下文块{retrieved_blocks_count}包含 {len(potential_block_messages)} 条消息 ({role_info}), 共 {block_chars} 字符")
                else:
        ...
        if not final_rag_messages:
            ...
            logger.info_color(
                ...
                TermColors.GREEN)
            logger.debug(
                f"RAG: 检索时间: {total_time_ms:.2f}ms, 平均每块: {total_time_ms / max(1, retrieved_blocks_count):.2f}ms, 总字符数: {total_chars}")
    ...
    def prepare_rag_messages(self, user_input: str) -> List[Dict]:
        ...
        if rag_prefix_content and rag_prefix_content.strip():
            ...
            if logger.should_print_context():
                logger.debug(
                    f"RAG: 添加前缀提示: \"{rag_prefix_content[:100]}\"{'...' if len(rag_prefix_content) > 100 else ''}")
        ...
        if rag_suffix_content and rag_suffix_content.strip():
            ...
            if logger.should_print_context():
                logger.debug(
                    f"RAG: 添加后缀提示: \"{rag_suffix_content[:100]}\"{'...' if len(rag_suffix_content) > 100 else ''}")
